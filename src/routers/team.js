...
const { auth, authMember } = require('../middleware/auth');
...
router.get('/teams/me/leader', auth, async (req, res) => {
    try {
        await req.user.populate('teams').execPopulate();
        const teams = req.user.teams.filter(item => item.leader.equals(req.user._id));
        res.send(teams);
    }
    catch (e) {
        console.log(e);
...
//
//  Updateovanje tima
router.patch('/teams/me', auth, async (req, res) => {
